...
| `langgraph` | LLM + Stack Exchange | M2 网页研究闭环 |
...
模型生成的 `search_query` 使用英文，是当前 Stack Overflow 技术检索的执行输入；界面仍展示原始研究问题。

## 4. 数据流

1. 模型生成 `ResearchPlan`；
2. Search Provider 为每个问题产生结果；
3. Page Reader 读取正文、清理 HTML、限制长度并按 URL 去重；
4. 模型提取 Evidence；
5. 模型写报告；
...
LLM 保持 `LLM_TIMEOUT`、`LLM_CONNECTION_ERROR`、`LLM_HTTP_ERROR`、`LLM_INVALID_RESPONSE` 等安全错误。网页层使用 `SEARCH_*`、`WEB_*` 错误；Stack Exchange 客户端只对超时、连接错误、429 和 5xx 做有限重试，不无限阻塞。原始响应正文和密钥不会进入用户事件。

## 6. 当前边界

- 当前网页 Provider 只覆盖 Stack Overflow 技术问答；
- 没有学术论文检索和 Claim 级引用验证；
