...
建议把 **Exa 作为第一候选，Tavily 作为第二候选**，在用户确认并创建免费 API Key 后，用同一组公开研究问题做一次真实对比冒烟测试，再决定最终实现。
...
1. 第一版选择 Exa，还是选择 Tavily？本文建议先比较二者的真实结果后优先 Exa。
2. 是否接受搜索查询被供应商用于服务改进？如果不接受，两者的普通免费层都不合适。
3. 注册免费账号取得 Key 后，使用同一组公开输入验证：
   - AI 代码生成工具评测方法；
   - 一项普通科研综述问题；
   - 一项企业技术选型问题。
4. 验证结果至少记录来源类型覆盖、重复率、正文可读率、中文输入效果、延迟和单次 credits。

在上述选择确认前，不删除现有实现，也不把候选方案写成已经上线的能力。
