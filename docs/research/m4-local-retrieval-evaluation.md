...
- 文档处理时调用 Gemini 原生 `batchEmbedContents`，按模型版本加入文档检索指令；
- 查询时使用同一模型、维度和指令策略，生成查询向量；
...
2026-08-26 使用 `gemini-embedding-2`、768 维向量运行同一组 3 条中文查询；按官方文档改为文本任务指令后再次复测，结果如下：
...
三个预期文档都排在第一位，说明修正后的任务指令、API 配置、SQLite 向量持久化和余弦排序能够共同跑通。样例规模很小，这类结果属于连通性与回归基线，不代表对真实大型知识库的通用质量结论。复现脚本会调用真实服务并消耗少量 Embedding 额度。
