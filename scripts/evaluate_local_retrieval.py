...
async def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="researchflow-retrieval-") as temporary:
        ...
        try:
            ...
            document_ids = await _load_samples(repository, sample_directory)
            evaluations = []
            for mode in RetrievalMode:
                retriever = LocalKnowledgeRetriever(repository, mode=mode)
                results = []
    ...
    best = max(evaluations, key=lambda item: (item["mrr_at_3"], item["top1_accuracy"]))
    print(
        json.dumps(
            {
    ...
    return 0 if best["top1_accuracy"] == 1 else 1
